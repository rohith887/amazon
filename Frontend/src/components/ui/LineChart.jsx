import { useState } from "react";

function cssVar(name) {
  return getComputedStyle(document.documentElement).getPropertyValue(name).trim();
}

export function LineChart({ data, series, width = 900, height = 380, padding = { top: 16, right: 20, bottom: 44, left: 44 } }) {
  const [hoverIndex, setHoverIndex] = useState(null);

  if (!data.length) {
    return (
      <div
        style={{
          height: `${height}px`,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          color: "var(--chart-muted)",
          fontSize: "13px",
        }}
      >
        No data available for the selected filters.
      </div>
    );
  }

  const innerWidth = width - padding.left - padding.right;
  const innerHeight = height - padding.top - padding.bottom;

  const maxY = Math.max(1, ...data.flatMap((d) => series.map((s) => d[s.key] ?? 0)));
  const yTicks = 5;

  const xStep = data.length > 1 ? innerWidth / (data.length - 1) : 0;
  const xFor = (i) => padding.left + i * xStep;
  const yFor = (v) => padding.top + innerHeight - (v / maxY) * innerHeight;

  const labelEvery = Math.ceil(data.length / 10) || 1;

  function onMove(e) {
    const rect = e.currentTarget.getBoundingClientRect();
    const scaleX = width / rect.width;
    const mouseX = (e.clientX - rect.left) * scaleX;
    let index = Math.round((mouseX - padding.left) / (xStep || 1));
    index = Math.max(0, Math.min(data.length - 1, index));
    setHoverIndex(index);
  }

  const hovered = hoverIndex != null ? data[hoverIndex] : null;
  const hoverX = hoverIndex != null ? xFor(hoverIndex) : 0;

  return (
    <div style={{ position: "relative", width: "100%" }}>
      <svg
        viewBox={`0 0 ${width} ${height}`}
        width="100%"
        height={height}
        style={{ display: "block", background: "var(--chart-surface)", borderRadius: "8px" }}
      >
        {Array.from({ length: yTicks + 1 }, (_, i) => {
          const value = Math.round((maxY / yTicks) * i);
          const y = yFor(value);
          return (
            <g key={i}>
              <line x1={padding.left} x2={width - padding.right} y1={y} y2={y} stroke="var(--chart-grid)" strokeWidth="1" />
              <text x={padding.left - 8} y={y + 4} textAnchor="end" fontSize="11" fill="var(--chart-muted)">
                {String(value)}
              </text>
            </g>
          );
        })}

        {data.map((d, i) => {
          if (i % labelEvery !== 0 && i !== data.length - 1) return null;
          return (
            <text
              key={i}
              x={xFor(i)}
              y={height - padding.bottom + 20}
              textAnchor="middle"
              fontSize="11"
              fill="var(--chart-muted)"
            >
              {d.name.length > 12 ? `${d.name.slice(0, 11)}…` : d.name}
            </text>
          );
        })}

        {series.map((s) => {
          const color = cssVar(s.colorVar);
          const points = data.map((d, i) => [xFor(i), yFor(d[s.key] ?? 0)]);
          const pathD = points.map((p, i) => `${i === 0 ? "M" : "L"}${p[0]},${p[1]}`).join(" ");
          return (
            <g key={s.key}>
              <path d={pathD} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              {points.map(([x, y], i) => (
                <circle key={i} cx={x} cy={y} r="3" fill={color} />
              ))}
            </g>
          );
        })}

        {hovered ? (
          <line x1={hoverX} x2={hoverX} y1={padding.top} y2={height - padding.bottom} stroke="var(--chart-axis)" strokeWidth="1" />
        ) : null}

        <rect
          x={padding.left}
          y={padding.top}
          width={innerWidth}
          height={innerHeight}
          fill="transparent"
          onMouseMove={onMove}
          onMouseLeave={() => setHoverIndex(null)}
        />
      </svg>

      {hovered ? (
        <div
          style={{
            position: "absolute",
            pointerEvents: "none",
            background: "#fff",
            border: "1px solid var(--chart-grid)",
            borderRadius: "8px",
            padding: "8px 10px",
            fontSize: "12px",
            boxShadow: "var(--shadow-popover)",
            minWidth: "140px",
            left: `${Math.min(100, Math.max(0, (hoverX / width) * 100))}%`,
            top: "8px",
          }}
        >
          <div style={{ fontWeight: "600", marginBottom: "4px", color: "var(--ink-900)" }}>{hovered.name}</div>
          {series.map((s) => (
            <div key={s.key} style={{ display: "flex", alignItems: "center", gap: "6px", color: "var(--ink-700)" }}>
              <span
                style={{
                  width: "8px",
                  height: "8px",
                  borderRadius: "50%",
                  background: cssVar(s.colorVar),
                  display: "inline-block",
                }}
              />
              {`${s.label}: ${hovered[s.key] ?? 0}`}
            </div>
          ))}
        </div>
      ) : null}

      <div
        style={{
          display: "flex",
          flexWrap: "wrap",
          gap: "16px",
          justifyContent: "center",
          marginTop: "8px",
          fontSize: "13px",
        }}
      >
        {series.map((s) => (
          <span key={s.key} style={{ display: "inline-flex", alignItems: "center", gap: "6px", color: "var(--chart-text-secondary)" }}>
            <span
              style={{
                width: "10px",
                height: "10px",
                borderRadius: "50%",
                background: cssVar(s.colorVar),
                display: "inline-block",
              }}
            />
            {s.label}
          </span>
        ))}
      </div>
    </div>
  );
}
